...
(function () {
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", {
        ...
        role: "tablist",
        "aria-label": "Kanban display mode",
        title: "Switch between the classic board and the workflow execution flow.",
      },
        ["board", "flow"].map(function (mode) {
          ...
          return h("button", {
            ...
            className: cn(
              "hermes-kanban-view-toggle-btn",
              active ? "hermes-kanban-view-toggle-btn--active" : "",
            ),
            onClick: function () { props.setViewMode(mode); },
          }, mode === "board" ? "Board" : "Flow");
        }),
      ),
      h("div", { className: "flex-1" }),
      h(Button, {
  ...
  const FLOW_ROLE_META = {
    ...
    dev: { label: "DEV", icon: "{}", tone: "dev" },
    ops: { label: "OPS", icon: "srv", tone: "ops" },
    hst: { label: "HST", icon: "www", tone: "hst" },
    inc: { label: "INC", icon: "!", tone: "inc" },
    sec: { label: "SEC", icon: "sec", tone: "sec" },
    mem: { label: "MEM", icon: "db", tone: "mem" },
    cnt: { label: "CNT", icon: "doc", tone: "cnt" },
  };

  function flowClassName(value) {
    return String(value || "unknown").toLowerCase().replace(/[^a-z0-9_-]+/g, "-");
  }

  function flowTaskRole(task) {
    const raw = String((task && task.assignee) || "").toLowerCase();
    const match = raw.match(/\b(orc|dev|ops|hst|inc|sec|mem|cnt)\b/);
    if (match) return match[1];
    return raw || "unassigned";
  }

  function flowRoleMeta(task) {
    const role = flowTaskRole(task);
    return FLOW_ROLE_META[role] || {
      label: role ? role.toUpperCase() : "AGT",
      icon: "AI",
      tone: "agent",
    };
  }

  function flowStatusMeta(status) {
    return FLOW_STATUS_META[status] || {
      label: String(status || "Unknown").replace(/_/g, " "),
      tone: flowClassName(status),
    };
  }
  ...
  function flattenFlowTasks(board) {
    const tasks = [];
    if (!board || !Array.isArray(board.columns)) return tasks;
    board.columns.forEach(function (column, columnIndex) {
      ...
      columnTasks.forEach(function (task, taskIndex) {
        tasks.push(Object.assign({}, task, {
          _flowColumn: column,
          _flowColumnIndex: columnIndex,
          _flowIndexInColumn: taskIndex,
          _flowStatus: task.status || column.name,
        }));
      });
    });
    return tasks;
  ...
  function WorkflowFlowView(props) {
    ...
    const applyZoom = useCallback(function (nextZoom, anchor) {
      ...
      if (Math.abs(clampedZoom - previousZoom) < 0.002) return;
      ...
      if (wrap) {
        const rect = wrap.getBoundingClientRect();
        anchorX = anchor ? anchor.x - rect.left : wrap.clientWidth / 2;
        anchorY = anchor ? anchor.y - rect.top : wrap.clientHeight / 2;
        originX = wrap.scrollLeft + anchorX;
        originY = wrap.scrollTop + anchorY;
      }
      ...
      if (wrap) {
        ...
        window.requestAnimationFrame(function () {
          wrap.scrollLeft = originX * ratio - anchorX;
          wrap.scrollTop = originY * ratio - anchorY;
        });
      }
    }, []);
    const handleFlowWheel = useCallback(function (event) {
    ...
    useEffect(function () {
      ...
      wrap.addEventListener("wheel", handleFlowWheel, { passive: false });
    ...
    return h("div", { className: "hermes-kanban-flow" },
      ...
      h("section", { className: "hermes-kanban-flow-stage" },
        h("div", { className: "hermes-kanban-flow-stage-header" },
          ...
          h("div", { className: "hermes-kanban-flow-header-actions" },
            ...
            h("div", { className: "hermes-kanban-flow-zoom", "aria-label": "Flow zoom controls" },
              h("button", {
                type: "button",
                className: "hermes-kanban-flow-zoom-btn",
                onClick: function () { applyZoom(zoomRef.current / 1.14); },
                title: "Zoom out",
              }, "-"),
              h("button", {
                type: "button",
                className: "hermes-kanban-flow-zoom-value",
                onClick: function () { applyZoom(1); },
                title: "Reset zoom",
              }, `${zoomPercent}%`),
              h("button", {
                type: "button",
            ...
            h("div", { className: "hermes-kanban-flow-legend" },
              ["triage", "todo", "scheduled", "ready", "running", "review", "blocked"].map(function (status) {
                ...
                return h("span", {
                  key: status,
                  className: cn("hermes-kanban-flow-legend-item", `hermes-kanban-flow-tone--${meta.tone}`),
                }, meta.label);
              }),
            ),
          ),
        ...
        h("div", {
          ...
          selectedCount === 0 ? h("div", { className: "hermes-kanban-flow-empty" },
            ...
            style: {
              width: `${flow.canvasWidth * zoom}px`,
              height: `${flow.canvasHeight * zoom}px`,
            },
          },
            h("div", {
              className: "hermes-kanban-flow-canvas",
              style: {
                width: `${flow.canvasWidth}px`,
                height: `${flow.canvasHeight}px`,
                transform: `scale(${zoom})`,
              },
            },
              h("svg", {
                className: "hermes-kanban-flow-wires",
                width: flow.canvasWidth,
                height: flow.canvasHeight,
                viewBox: `0 0 ${flow.canvasWidth} ${flow.canvasHeight}`,
                role: "presentation",
                "aria-hidden": "true",
              },
